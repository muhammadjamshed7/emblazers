...
- **Student**: Logs in via Student ID + password at `POST /api/student-portal/login`. Default password is DOB in DDMMYYYY format (bcrypt hashed). Portal accounts created by admin. Gets dashboard, study material, quizzes, results, fees, and attendance pages. Student routes: `/api/student-portal/dashboard`, `/api/student-portal/content` (grouped by subject), `/api/student-portal/quizzes` (with status: upcoming/active/expired + attempted flag), `/api/student-portal/quizzes/:id/start` (returns quiz without answers), `/api/student-portal/quizzes/:id/submit`, `/api/student-portal/results` (joined with quiz title/subject/teacher), `/api/student-portal/fees`, `/api/student-portal/attendance`, `/api/student-portal/change-password`.
